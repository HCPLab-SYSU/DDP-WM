...
class Trainer:
    def __init__(self, cfg):
        ...
        if self.accelerator.is_main_process:
            ...
            if os.path.exists("hydra.yaml"):
                ...
                wandb_run_id = existing_cfg.get("wandb_run_id", None) # Use .get avoid KeyEroor
                if wandb_run_id:
        ...
        self.datasets, traj_dsets = hydra.utils.call(
            ...
            num_hist=self.cfg.num_hist if self.cfg.predictor.training_stage=='localization' else 5,
        ...
        if self.cfg.ckpt_path:
            model_ckpt = Path(self.cfg.ckpt_path)
            if model_ckpt.exists():
                log.info(f"Loading checkpoint from: {model_ckpt}")
                self.load_ckpt(model_ckpt)
            else:
                raise FileNotFoundError(f"Checkpoint not found at: {self.cfg.ckpt_path}")
    ...
    def save_ckpt(self):
        ...
        if self.accelerator.is_main_process:
            ...
            # Minimal dataset information required to save the instantiated model
            try:
                ckpt['dataset_info'] = {
                    'proprio_dim': self.datasets["train"].proprio_dim,
                    'action_dim': self.datasets["train"].action_dim,
                }
            except Exception as e:
                log.error(f"Failed to get dataset dims: {e}")
                log.warning("Checkpoint will be saved WITHOUT dataset_info.")
            ...
            for k in self._keys_to_save:
                ...
                if hasattr(obj_to_save, "module"):
                    unwrapped_obj = self.accelerator.unwrap_model(obj_to_save)
                else:
                    unwrapped_obj = obj_to_save
                
                # Save state_dict
                if hasattr(unwrapped_obj, "state_dict"):
                    ckpt[k] = unwrapped_obj.state_dict()
                else:
                    # just in case, but according to _keys_to_save This part should not be reached
                    log.warning(f"Object {k} has no state_dict, saving it directly.")
                    ckpt[k] = unwrapped_obj

            # Save
            torch.save(ckpt, "checkpoints/model_latest.pth")
            torch.save(ckpt, f"checkpoints/model_{self.epoch}.pth")
            log.info("Saved model state_dict to {}".format(os.getcwd()))
    ...
    def load_ckpt(self, filename="model_latest.pth"):
        log.info(f"Loading checkpoint state_dict from {filename}...")
        ckpt = torch.load(filename, map_location=self.device) # Ensure loading to the correct device
        
        not_in_ckpt = []
        
        for k in self._keys_to_save:
            if k not in ckpt:
                not_in_ckpt.append(k)
                continue
            ...
            try:
                if hasattr(obj_to_load, "load_state_dict"):
                    is_model = not k.endswith("optimizer")
                    
                    if is_model:
                        unwrapped_model = self.accelerator.unwrap_model(obj_to_load)
                        unwrapped_model.load_state_dict(value, strict=False)
                    else:
                        obj_to_load.load_state_dict(value, strict=False)
                else:
                    log.warning(f"Object {k} has no load_state_dict, attempting direct assignment.")
                    self.__dict__[k] = value
            except Exception as e:
                log.error(f"Failed to load state_dict for key '{k}'. Error: {e}")
                log.warning(f"Skipping loading for '{k}'. This might be expected if model architecture changed.")

        if len(not_in_ckpt):
            log.warning("Keys not found in ckpt (this is normal if resuming from an older version): %s", not_in_ckpt)
        
        log.info(f"Checkpoint state_dict loaded successfully. Resuming from epoch {self.epoch}.")
    ...
    def init_models(self):
        ...
        if self.cfg.has_predictor:
            if self.predictor is None:
                self.predictor = hydra.utils.instantiate(
                    ...
                    d_visual=self.encoder.emb_dim,
                    d_proprio_embed = proprio_emb_dim,
                    d_action_embed = action_emb_dim,
                )
        ...
        self.model = hydra.utils.instantiate(
            ...
            label_generator=hydra.utils.instantiate(self.cfg.label_generator),
    ...
    def openloop_rollout(
        ...
        for idx in range(num_rollout):
            ...
            for attempt in range(max_attempts):
                ...
                if rand_start_end:
                    ...
                    if max_horizon > min_horizon:
                        ...
                        break # break once found for loop
                else:
                    valid_traj = True
                    start = 0
            ...
            if not valid_traj:
                # If it's still not found after many attempts，then skip this one rollout or raise an error
                print(f"Warning: Could not find a valid trajectory after {max_attempts} attempts. Skipping this rollout.")
                continue # skip to the next one num_rollout
            for k in obs.keys():
                obs[k] = obs[k][
                    start : 
    ...
    def logs_flash(self, step):
        ...
        log.info(f"Validation results for epoch {self.epoch}: Validation loss: {epoch_log.get('val_loss', np.nan):.4f}")
